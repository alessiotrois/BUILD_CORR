...
int Open3916(int argc, char ** argv, fitsfile ** output, char * infileTCS) {
	...
	printf("####################################################\n");
	printf("#########  Task Cor_tcs......... starting  #########\n");
	printf("#########         %s         #########\n", version);
	printf("####################################################\n\n");
	printf("Cor_tcs..................... starting Open3916\n");
	...
	PILGetString("infileTCS",infileTCS);	
	...
	int tfields = 5;       /* table will have 20 columns */
		
	char *ttype[] = { "TIME","TCTYPE","TCSTYPE","DATE","DESCR"};
	char *tform[] = { "1D","1I","1I","32A","32A"};
	char *tunit[] = { "s","","","",""};
	
	
	fits_create_tbl( output[0], BINARY_TBL, 1, tfields, ttype, tform,   tunit, "TCS", &status);
	delete []outfile;		
		
	
	
	if ( status) {
		printf("Cor_tcs..................... exiting Cor_tcs input ERROR:");		
		fits_report_error(stdout, status);
		printf("\n");
		return status;					
		}
	else {
		printf("Cor_tcs..................... exiting Cor_tcs\n");		
	
		}
	return status;
	}

int CorrTime(fitsfile * output, char * infileTCS) {

	int status = 0;
	printf("\nCor_tcs..................... starting CorrTime\n");
...
// 	float bw = 0;
	...
	while (fgets(buffer , 2048, fp)) {
		...
		if (strcmp(Ptr,"Description")==0)  	{ 		
			...
			fits_get_colnum(output, 1, "DESCR", &colnum, &status);
			...
			Ptr = strtok(buffer,":");
			Ptr = (strtok(NULL,""));			
			short stype = atoi(Ptr);
			

			fits_get_colnum(output, 1, "TCTYPE", &colnum, &status);
			fits_write_col(output, TSHORT, colnum, i+1, 1, 1, &type, &status);	
			fits_get_colnum(output, 1, "TCSTYPE", &colnum, &status);
			fits_write_col(output, TSHORT, colnum, i+1, 1, 1, &stype, &status);	
...
// 			cout<<"TC NUMERO => "<<i<<endl;
	...
	if (i == 0) {
		printf("Cor_tcs..................... exiting CorrTime ERROR:");	
		printf("FILE TCS EMPTY");	
		printf("\n");
		printf("\n");			
		status=-1;
		return status;			
...
///////////////////////////////////////////////////////////////////////////////////////////////	
	...
	if ( status) {
		...
		printf("Cor_tcs..................... exiting CorrTime STATUS = 0\n\n");		
		}	
...
int Close3916(int argc, char ** argv, fitsfile * output) {
	...
	printf("Cor_tcs..................... starting Close3916\n");	
...
// 	cout<<argv[2]<<endl;
	if( argc==4) {
		FILE *fp4;
		if(checkfile(argv[3], &argv[2][1], tmin, tmax, (char *)packet) == 0) {
			fp4 = fopen(argv[3], "a+");
// 			cout<<argv[2]<<endl;
			if (argv[2][0] == '!')
	...
	if ( status) {
		printf("Cor_tcs..................... exiting Close3916 input ERROR:");		
		fits_report_error(stdout, status);	
		printf("\n");
		return status;
		}			
	...
	printf("Cor_tcs..................... exiting Close3916 STATUS = 0\n\n");		
...
char * infileTCS = new char[FLEN_KEYWORD];


status = Open3916(argc, argv, output, infileTCS);

// cout<<infileSOE<<endl;

 if (status ==0)
 	status = CorrTime(output[0],infileTCS);
